...
// Funktion zum Auslesen der internen Alben direkt aus dem Google Drive Ordner
async function fetchInternalAlbumsFromDrive(): Promise<AlbumItem[]> {
  const rootFolderId = process.env.PUBLIC_INTERNAL_PICTURES_ROOT_ID;
  if (!rootFolderId) {
    console.error("PUBLIC_INTERNAL_PICTURES_ROOT_ID ist nicht in der .env definiert.");
    return [];
  }
  ...
  try {
    ...
    // 1. Alle Unterordner (interne Alben) im Root-Ordner abrufen
    ...
    const albumPromises = subFolders.map(async (folder): Promise<AlbumItem | null> => {
      ...
      return {
        id: folder.id,
        titel: folder.name,
        link: `/intern/bilder/${folder.id}`, // Angepasst auf den internen Pfad
        vorschaubild,
        beschreibung: folder.description || `Internes Fotoalbum mit ${images.length} Bildern`,
    ...
    console.error("Fehler beim Laden der internen Alben aus Google Drive:", error);
    return [];
  }
}

export default async function InternalBilderPage() {
  const albums = await fetchInternalAlbumsFromDrive();
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-white/10 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-black text-white">Interne Fotoalben</h1>
          <p className="text-xs text-indigo-200/70 mt-0.5">Exklusive Einblicke und Mitschnitte für Chormitglieder</p>
        </div>
      ...
      {albums.length === 0 ? (
        <div className="bg-black/30 backdrop-blur-md p-8 rounded-3xl border border-white/10 text-center text-indigo-200/70 text-sm">
          Aktuell sind keine internen Alben im Google Drive Verzeichnis hinterlegt.
        </div>
      ) : (
